...
- 已完成 M7 信标链路代码层：`BEACON` 模式装置右键投掷 `StratagemBeaconProjectile`、命中后生成 `StratagemMarkerEntity`、marker 通过维度/路径阻隔校验后启动玩家战备冷却。长按呼叫装置只在有天花板维度或明显低于地表的洞穴环境阻止输入，普通上方遮挡交给信标落点路径检测。
- M7 已经过游戏内输入/信标流程验证并做过多轮小修；marker renderer、名称/倒计时和光柱已有第一版，当前 marker 信息牌使用稳定的文本图标占位、名称和倒计时/着弹中状态。
- 已完成 M8 CBC 发射适配第一版代码层：`compat/cbc/CbcProjectileLauncher` 通过 CBC `ProjectileBlock#getProjectile(level, stack)` 生成炮弹，marker 倒计时结束后按 `artillery[].count` 和 `interval_ticks` 发射。带引信的 CBC 炮弹如果没有 `FUZE` 组件，会自动补 `createbigcannons:impact_fuze`。
- M8 已做过游戏内初步验证：炮弹能垂直落地；后续根据验证反馈提高默认 `power` 到 `8.0`，默认 `max_obstruction_blocks` 到 `16`，并让 marker 在预计落地前约 10 tick 才消失。`artillery` 支持 `trajectory_mode`：默认 `auto` 会在半径 56、最小仰角 30 度的离散候选路径中选择阻隔跨度最小路径；`fixed` 使用 `spawn_bearing_degrees` + `spawn_distance` 固定水平发射方位。仍需复测 HE shell 命中爆炸、速度手感和 marker 消失时机。
...
随后再复测 CBC 发射爆炸闭环、发射失败细化处理、marker 视觉细节调优和完整火力链路验证。
...
M8 CBC 发射适配第一版代码层已完成并通过 ./gradlew build；已初步验证炮弹可生成/落地，并已补自动 impact fuze、默认 power=8.0、marker 延迟消失逻辑。
请先复测露天/阻隔拒绝逻辑和 CBC 发射爆炸闭环，再继续发射失败细化处理、marker 视觉细节调优和完整火力链路验证。
```
