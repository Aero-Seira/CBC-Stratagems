...
- 已完成 M7 信标最小链路代码层：`BEACON` 模式装置右键投掷 `StratagemBeaconProjectile`、命中后生成 `StratagemMarkerEntity`、marker 启动玩家战备冷却、倒计时结束后提示炮击调度尚未实现。
- M7 尚未做游戏内手动验证；marker 目前只有占位 renderer，不显示光柱/名称/倒计时；CBC 炮击调度尚未实现。
...
随后再做 marker 光柱/名称/倒计时渲染、阻隔校验和 CBC 发射适配。
...
M7 信标投掷、marker 生成、倒计时和冷却启动代码层已完成并通过 ./gradlew build，但还需游戏内手动验证。
请先验证 M4-M7 解锁、输入、信标投掷和 marker 倒计时闭环，再继续 marker 光柱/名称/倒计时渲染、阻隔校验和 CBC 发射适配。
```
